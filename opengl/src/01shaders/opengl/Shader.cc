...
inline Shader::Shader(const char shaderName[],
  ...
  try {
    ...
    if (geometryPath != nullptr) {
      geometryShaderID = load ?
				loadAndCompile(geometryPath, GL_GEOMETRY_SHADER, "GEOMETRY") :
				justCompile(geometryPath, GL_GEOMETRY_SHADER, "GEOMETRY");
...
Shader Shader::loadCompileAndLink(const char shaderName[],
																	const char vertexPath[],
																	const char fragmentPath[],
																	const char geometryPath[]) {
