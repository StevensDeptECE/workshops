...
public:
	...
	void write(const char filename[]) {
		...
		//TODO: note, optimally write a single block of memory a multiple of block size
		// this writes capacity, size, and head
		f.write((char*)&capacity, 3*sizeof(uint32_t));
		f.write((char*)nodes, size*sizeof(Node));
	}
	// constructor to read the file back in
	FastLinkedList(const char filename[]) {
		ifstream f(filename, ios::binary);
		f.read((char*)&capacity, 3* sizeof(uint32_t));
		nodes = (Node*)malloc(capacity*sizeof(Node));
		f.read((char*)nodes, size*sizeof(Node));
	}
};
